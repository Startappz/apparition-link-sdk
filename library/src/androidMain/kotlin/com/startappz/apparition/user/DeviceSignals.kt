    ...
    return withContext(Dispatchers.Default) {
        mutex.withLock {
            ...
            if (!TextUtils.isEmpty(ApparitionLinkSDK.userAgentString)) {
                ApLogger.v("UserAgent cached " + ApparitionLinkSDK.userAgentString)
                result = ApparitionLinkSDK.userAgentString
            } else {
                try {
                    ApLogger.v("Begin getUserAgentAsync " + Thread.currentThread())
                    result = WebSettings.getDefaultUserAgent(context)
                    ApLogger.v("End getUserAgentAsync " + Thread.currentThread() + " " + result)
                } catch (exception: Exception) {
                    ApLogger.e("Failed to retrieve userAgent string. " + exception.message)
                }
            }
...
suspend fun getUserAgentSync(context: Context): String? {
    return withContext(Dispatchers.Main) {
        ...
        if (!TextUtils.isEmpty(ApparitionLinkSDK.userAgentString)) {
            ...
            try {
                ApLogger.v("Begin getUserAgentSync " + Thread.currentThread())
                val w = WebView(context)
                result = w.settings.userAgentString
                w.destroy()
                ApLogger.v("End getUserAgentSync " + Thread.currentThread() + " " + result)
            } catch (ex: Exception) {
                ApLogger.e("Failed to retrieve userAgent string. " + ex.message)
            }
        }
